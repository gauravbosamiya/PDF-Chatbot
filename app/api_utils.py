...
def list_document():
    try:
        ...
        if response.status_code==200:
            ...
            st.error(f"Faild to fetch document list. Error: {response.status_code} - {response.text}")
            return []
    except Exception as e:
...
def delete_document(file_id: int):
    ...
    try:
        response = requests.delete("http://localhost:8000/delete-doc", headers=headers, json=data)
        if response.status_code == 200:
            return response.json()
        else:
